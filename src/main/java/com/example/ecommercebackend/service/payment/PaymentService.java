        ...
        if (payCallBackDto.getStatus().equals("success")) {
            ...
            if (order.getUser() != null){
                if (order.getUser() instanceof Customer customer){
                    customer.getCard().getItems().clear();
            ...
            mailService.send(order.getUsername(),"Siparişiniz onaylandı",buildOrderConfirmationEmail(order,order.getShippingFee()));
...
Kuruş farkı toleransı aşan geçersiz istek
    ...
    public String buildOrderConfirmationEmail(Order order, BigDecimal shippingPrice) {
            ...
            <td style="padding:24px 30px;color:#333333;">
              ...
              <table role="presentation" width="100%%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:18px;">
                ...
                <tbody>
                  %s
                  <tr>
                    <td style="padding:8px;border:1px solid #e9ecef;"><strong>Kargo</strong></td>
                    <td style="padding:8px;border:1px solid #e9ecef;text-align:right;">₺%.2f</td>
                  </tr>
                  <tr>
...
""".formatted(
                ...
                order.getCustomerPrice(),        // Ürün toplam tutarı
                orderDate,                       // Tarih
                itemsRows,                       // Ürün satırları
                shippingPrice,                   // Kargo ücreti
                order.getCustomerPrice(), // Toplam (ürün + kargo)
                email,                           // E-posta
                email,                           // E-posta
